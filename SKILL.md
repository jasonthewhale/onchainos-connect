...
description: Pair the Onchain OS WalletConnect agent with a dApp. Subcommands "connect <wc-uri>", "disconnect <dapp-name>", "list", "status", "stop". The agent runs as a headless daemon that auto-spawns on first connect and auto-exits a few seconds after the last session closes.
